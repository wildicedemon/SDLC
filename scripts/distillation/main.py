...
def main(research_file: str):
    ...
    p1_response = client.generate(system_prompt, p1_user_prompt) if client else get_mock_response(1)
    ...
    p23_response = client.generate(system_prompt, p23_user_prompt) if client else get_mock_response(2)
    ...
    try:
        ...
        if isinstance(p23_data, dict):
            ...
            print("Warning: Prongs 2 & 3 output is not a dictionary. Saving as raw YAML.")
    ...
    p4_response = client.generate(system_prompt, p4_user_prompt) if client else get_mock_response(4)
