"""Path-based domain and capability classifier for research artifacts.

Maps a repository-relative file path to a :class:`Classification`
containing domain tags (e.g. ``context_management``) and optional
capability tags derived from the filename.  The classifier recognises:

1. Research directories under ``docs/research/`` and its
   ``Kimi-Research/`` mirror, including numbered domain folders,
   special ``_distilled`` / ``_extractions`` directories, and
   meta/indices areas.
2. Top-level knowledge prefixes such as ``distilled/`` and
   ``docs/distillation/``.

Unrecognised paths receive an empty :class:`Classification` with
``is_classified=False``.
"""
...
# ---------------------------------------------------------------------------
# Domain directory taxonomy — maps top-level numbered directories to
# the list of valid sub-domain directory names they may contain.
# ---------------------------------------------------------------------------
...
# Special directories inside a research root (underscore-prefixed or numbered)
...
def _make_research_patterns(prefix: str) -> tuple[re.Pattern[str], re.Pattern[str], re.Pattern[str], re.Pattern[str]]:
    """Build compiled regex patterns for a single research prefix.

    Returns four patterns that match, in order:
    * Standard domain path  (``prefix/NN_domain/subdomain/file``)
    * Meta directory         (``prefix/00_meta/...``)
    * Indices directory      (``prefix/03_indices/...``)
    * Special underscore dir (``prefix/_xxx/...``)
    """
...
# Pre-compiled patterns for every research prefix
_RESEARCH_PATTERNS = [
...
class Classification:
    ...
    Attributes:
        domain_tags: Domain labels derived from the path hierarchy.
        capability_tags: Capability labels derived from the filename.
        is_classified: ``True`` if the path matched any known pattern.
    """
...
def _capability_from_filename(filename: str) -> str | None:
    """Derive a capability tag from a filename stem.

    Generic filenames like ``overview``, ``patterns``, or ``README``
    are excluded and return ``None``.

    Args:
        filename: The basename of the file (with or without extension).

    Returns:
        The filename stem as a capability tag, or ``None`` if generic.
    """
...
def classify(path: str) -> Classification:
    """Classify a repository-relative file path into domain and capability tags.

    The function walks a priority-ordered set of rules:

    1. Research prefixes (``docs/research/``, ``Kimi-Research/docs/research/``),
       checking meta dirs, indices dirs, special underscore dirs, and
       standard ``NN_domain/subdomain`` paths.
    2. Top-level knowledge prefixes (``distilled/``, ``distillation/``, etc.).
    3. Falls through to an empty :class:`Classification` if nothing matches.

    Args:
        path: A repository-relative file path (forward- or back-slash).

    Returns:
        A :class:`Classification` with populated tags if recognised.
    """
